...
import { ParsedDocument, AppSettings, ColorMode, Tab, DocumentType, ChatMessage, PineXAction, ReaderControlMode } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    if (Capacitor.isNativePlatform()) {
        const handler = CapacitorApp.addListener('backButton', ({ canGoBack }) => {
            ...
            // Navigation Hierarchy: Anything other than DOCUMENTS goes back to DOCUMENTS
  ...
  }, [activeTab, currentDoc, backPressCounter, handleTabChange, showPrivacy, showOnboarding]);

  // INITIALIZATION LOGIC
  useEffect(() => {
      // 1. Check Privacy Agreement
      const privacyAgreed = localStorage.getItem('privacyAgreed');
      
      if (privacyAgreed === 'true') {
          setShowPrivacy(false);
          // 2. Check Onboarding
          const onboardingComplete = localStorage.getItem('onboardingComplete_v2');
      ...
      refreshRecentFiles();
      const handlePopState = (event: PopStateEvent) => { if (currentDoc) handleCloseDocumentFixed(); };
  ...
  const handlePrivacyAgreed = () => {
      ...
      localStorage.setItem('privacyAgreedDate', Date.now().toString());
      setShowPrivacy(false);
      setShowOnboarding(true);
  };

  const handleOnboardingComplete = () => {
      localStorage.setItem('onboardingComplete_v2', 'true');
      setShowOnboarding(false);
  };
  ...
  const handleTranslateArticle = async () => {
    ...
    try {
        ...
        const newDoc: ParsedDocument = {
            ...currentDoc,
            metadata: { ...currentDoc.metadata, name: translatedTitle },
            pages: [{
                ...currentDoc.pages[0],
  ...
  const handleShare = async () => {
      ...
      try {
          if (currentDoc.metadata.type === DocumentType.WEB || currentDoc.metadata.type === DocumentType.WEB_ARTICLE) {
             ...
             if (Capacitor.isNativePlatform()) {
                 await CapacitorShare.share({ title: shareData.title, text: shareData.text, dialogTitle: 'Share Article' });
  ...
  const handleSummarizeDocument = async () => {
      ...
      try {
          ...
          setSummaryResult(summary);
          const updatedDoc = { ...currentDoc, metadata: { ...currentDoc.metadata, summary } };
  ...
  const isDocOpen = activeTab === Tab.DOCUMENTS && !!currentDoc;
  const isHighContrast = settings.colorMode === ColorMode.HIGH_CONTRAST;

  // Render logic based on new tab structure
  const renderContent = () => {
      // 1. Mandatory Privacy Agreement
      if (showPrivacy) {
          return <PrivacyAgreementModal settings={settings} onAgree={handlePrivacyAgreed} />;
      }

      // 2. Onboarding Carousel
      if (showOnboarding) {
          return <OnboardingCarousel settings={settings} onComplete={handleOnboardingComplete} />;
      }
                    ...
                    return (
                      <div className="flex flex-col h-full bg-black">
                          ...
                          {selectedText && !summaryResult && !isSummarizing && (
                             <div className="absolute bottom-28 left-1/2 -translate-x-1/2 z-[55] animate-in zoom-in-90 duration-300 w-full px-4 flex justify-center">
                                 <Button 
                                    label="Summarize Selection"
                                    onClick={handleSummarize}
                                    colorMode={settings.colorMode}
                                    className={clsx(
                                        "shadow-2xl text-lg font-bold rounded-full border-2 w-auto",
            ...
            case Tab.OCR:
                return (
                  <Suspense fallback={<TabLoader />}>
                    <OCRView 
                      settings={settings} 
                      onBack={() => handleTabChange(Tab.MORE)} 
                    />
                  </Suspense>
                );
            case Tab.PINEX:
                return (
                  <Suspense fallback={<TabLoader />}>
                    <PineX 
                      ...
                      settings={settings} 
                      isEmbedded={false} // Full screen with back
                      onControlAction={handlePineXControl} 
                      messages={pineXMessages} 
                      onUpdateMessages={setPineXMessages} 
                      onBack={() => handleTabChange(Tab.DOCUMENTS)} // Navigates back to Home
                    />
                  </Suspense>
                );
            case Tab.BOOKMARKS:
                return (
                  <Suspense fallback={<TabLoader />}>
                    <BookmarksView 
                        ...
                        onBack={() => handleTabChange(Tab.MORE)} // Navigates back to More
                    />
                  </Suspense>
                );
            case Tab.SETTINGS:
  ...
  return (
    ...
    >
      ...
      <div className={clsx("flex-1 flex flex-col relative overflow-hidden", isDocOpen && !audioModeActive && activeTab === Tab.DOCUMENTS && "pb-[0px]")}>
          {renderContent()}
      </div>
      
      {/* GLOBAL NAVIGATION - Only on Home/Docs Tab */}
      {!showPrivacy && !showOnboarding && !isDocOpen && activeTab === Tab.DOCUMENTS && (
          <BottomNav 
